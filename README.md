...
`pnpm check` runs strict TypeScript (including checked JavaScript hooks), lint, architecture rules, fixture/seed integrity and tests. It executes non-synthetic discovery cases when present. At setup it explicitly reports:
...
- `ai/`, `scripts/`, `.codex/hooks.json`: portable role briefs, setup prompt, supplied evidence and Codex execution controls.
...
Antoine selects `./use-role.sh impl` or `./use-role.sh verify`, then opens a fresh Codex session using the printed command. Never resume/fork the other role's conversation or run the roles concurrently. `./use-role.sh none` removes only generated local role files. Both role briefs are portable; configuration and hooks are Codex-specific. The `.mjs` bootstrap/hook files run directly in Node and are checked under the same strict TypeScript settings through JSDoc and `checkJs`.

Complete the real-session permission checks in `SETUP-NOTES.md` before entrusting reserved evidence to this environment. A native sandbox test and a unit-tested hook are not proof that hooks loaded in a live session. The verifier can execute engine code; its non-inspection rule is workflow independence, not inviolable system isolation. Unlisted role commands are refused. The operator commits outside the role session after `pnpm check`.

## Verification limits

No merchant request, new capture, reference container or business implementation occurred during setup. The target seed is empty, the rules document is a template and the write-up contains only headings. The later lab seed must independently exercise two combinable percentage coupons; that will not establish merchant support for coupon stacking.

Neither role may read held-out evidence, run `pnpm heldout` or reseal fixtures. During a role session, integrity checks enumerate discovery and seeds only and refuse a manifest containing published held-out paths; they do not attempt to read the denied held-out directory. Outside roles, integrity checks include published held-out files without replaying their official evaluation. Roles cannot be activated after held-out publication.
...
The operator-only [delivery procedure](docs/delivery.md) separates `prepare` and `finalize`, freezes code and configuration, reviews imported evidence, commits packaging before a fresh clone audit, and scans history plus delivered files. It has not been run on this setup. Gitleaks is a local prerequisite, not an npm dependency. No agent pushes.
